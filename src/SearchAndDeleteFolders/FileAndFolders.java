...
public class FileAndFolders {
	...
	private static void deleteAllFoldersForMatchingRegex(String workSpace, String regex){
		...
		FileUtils.listFilesAndDirs(new File(workSpace), new WildcardFileFilter("*"), TrueFileFilter.INSTANCE) // workspace param
				...
				.forEach(b -> Arrays.stream(Objects.requireNonNull(b.listFiles(x -> (x.lastModified() < Long.parseLong(LocalDate.now().minusMonths(0).format(DateTimeFormatter.ofPattern("yyyyMMdd")))
				))))
						.forEach(c -> {
							try {
